...
void Write_Output (int n_step, int n_cycle, int num_step, int mark_interval, 
           ...
           struct sphere *spheres, struct monomer *monomers, struct face *faces, 
           struct sphere_param *sphere_pm, Float ***velcs_df, int **node_map, 
           char *work_dir)
{
	/*
		 static int index;
	...
	//  if(num_sph != 0) // num_sph=0
	//    Write_Particle(objects, sphere_pm, n_step, work_dir);
	if(sphere_pm->num_beads !=0) {
		...
		Write_Sphereconfig (spheres, monomers, faces, sphere_pm, n_step, n_cycle*num_step, 
				                work_dir);
		//Write_RDF(spheres, monomers, sphere_pm, n_step, n_cycle*num_step, num_step, work_dir);
	}
...
//	outfile = fopen(FILE_NAME, "a");
	for(k=1; k <= max_z; k++) 
  {
    //fprintf(outfile, "VECTORS velSlice(z=%d) double      \n", k); // 20161228
        ...
        for(d=0; d<3; d++)
          v[d] /= 36.0;  // divided by fluid density
		...
		double fluidStress_pos[6])
{
    ...
    //double stress_v2[3][3];
		...
		for(int n=0; n < beadNum; n++) {
          ...
          stress_area[i][j] += monomers[n].stress_area[i][j];
        ...
        stress_area[1][0], stress_wall[1][0], stress_int[1][0]);
		fclose(stream);
...
//20161129
	...
	if(n_step % sphere_pm->write_time == 0) 
  {
    ...
    double avg_dr=0.; // 20161129
      ...
      monomers[n].dr = sqrt(monomers[n].dr2); // 20161129
      avg_dr += monomers[n].dr; // 20161129
			avg_dr2 += monomers[n].dr2;
      avg_vel += sqrt(temp_vel); 
		}
    avg_dr /= num_beads; // 20161129
		avg_disp2 /= num_beads;
		avg_dr2 /= num_beads;
    avg_vel /= num_beads;
...
void WriteForce(int step, struct sphere_param *sphere_pm, struct monomer *mono, 
     char *work_dir)
{
  double force_ev[DIMS] = {0.};
  double force_face[DIMS] = {0.};
  double force_spring[DIMS] = {0.};
  double force_bending[DIMS] = {0.};
  double force_vol[DIMS] = {0.};
  double force_area[DIMS] = {0.};
  double force_total[DIMS] = {0.};
  double force_drag[DIMS] = {0.};
  double force_wall[DIMS]={0.};
  double force_fluc[DIMS]={0.};
  double mag_force_ev=0.;
  double mag_force_face=0.;
  double mag_force_spring=0.;
  double mag_force_bending=0.;
  double mag_force_vol=0.;
  double mag_force_area=0.;
  double mag_drag=0.;
  double mag_force_wall=0.;
  double mag_force_fluc=0.;
  double mag_force_total=0.;
  char filename[200];
  FILE *stream;
  int totalBead = sphere_pm->num_beads;
  sprintf(filename, "%s/data/force_single.dat", work_dir);
  
  stream = fopen(filename, "a"); 
  for(int n=0; n < totalBead; n++) 
  {
    double single_ev=0.;
    double single_face=0.;
    double single_spring=0.;
    double single_bending=0.;
    double single_vol=0.;
    double single_area=0.;
    double single_drag=0.;
    double single_wall=0.;
    double single_fluc=0.;
    double single_force_total=0.;
    double single_total[DIMS]={0.};
    
    for(int i=0; i < DIMS; i++) {
      single_ev += mono[n].force_ev[i] * mono[n].force_ev[i];
      single_face += mono[n].force_face[i] * mono[n].force_face[i];
      single_spring += mono[n].force_spring[i] * mono[n].force_spring[i];
      single_bending += mono[n].force_bend[i] * mono[n].force_bend[i];
      single_vol += mono[n].force_vol[i] * mono[n].force_vol[i];
      single_area += mono[n].force_area[i] * mono[n].force_area[i];
      single_drag += mono[n].drag[i] * mono[n].drag[i];
      single_wall += mono[n].force_wall[i] * mono[n].force_wall[i];
      single_fluc += mono[n].force_fluc[i] * mono[n].force_fluc[i];
      single_total[i] = mono[n].force_ev[i] + mono[n].force_face[i] + 
                       mono[n].force_spring[i] + mono[n].force_bend[i] + 
                       mono[n].force_vol[i] + mono[n].force_area[i] + 
                       mono[n].drag[i] + mono[n].force_wall[i] + mono[n].force_fluc[i];
    }
    single_ev = sqrt(single_ev);
    single_face = sqrt(single_face);
    single_spring = sqrt(single_spring);
    single_bending = sqrt(single_bending);
    single_vol = sqrt(single_vol);
    single_area = sqrt(single_area);
    single_drag = sqrt(single_drag);
    single_wall = sqrt(single_wall);
    single_fluc = sqrt(single_fluc);
    for(int i=0; i < DIMS; i++)
      single_force_total += single_total[i]*single_total[i];
    single_force_total = sqrt(single_force_total);

    fprintf(stream, "%6d  ",step);
    fprintf(stream, "%+.4le    %+.4le    %+.4le    %+.4le   %+.4le    %+.4le    %+.4le    %+.4le    %+.4le    %+.4le\n",
            single_ev, single_face, single_spring, single_bending, single_vol, 
            single_area, single_drag, single_wall, single_fluc, single_force_total);
  }
  fclose(stream);

  for(int n=0; n < totalBead; n++) {
    for(int i=0; i < DIMS; i++) {
      force_ev[i] += mono[n].force_ev[i]; 
      force_face[i] += mono[n].force_face[i];
      force_drag[i] += mono[n].drag[i];
      force_spring[i] += mono[n].force_spring[i];
      force_bending[i] += mono[n].force_bend[i];
      force_vol[i] += mono[n].force_vol[i];
      force_area[i] += mono[n].force_area[i];
      force_wall[i] += mono[n].force_wall[i];
      force_fluc[i] += mono[n].force_fluc[i];
      // i don't take wall force and thermal fluctuation into account.
    }
  }
  for(int i=0; i < DIMS; i++) 
    force_total[i] = force_ev[i] + force_face[i] + force_drag[i] + force_spring[i] + 
                     force_bending[i] + force_vol[i] + force_area[i] + force_wall[i] + 
                     force_fluc[i];
  for(int i=0; i < DIMS; i++) {
    mag_force_ev +=      (force_ev[i]*force_ev[i]);
    mag_force_face += (force_face[i]*force_face[i]);
    mag_force_spring +=   (force_spring[i]*force_spring[i]);
    mag_force_bending +=  (force_bending[i]*force_bending[i]);
    mag_force_vol += (force_vol[i]*force_vol[i]);
    mag_force_area += (force_area[i]*force_area[i]);
    mag_drag += (force_drag[i]*force_drag[i]);
    mag_force_wall += (force_wall[i]*force_wall[i]);
    mag_force_fluc += (force_fluc[i]*force_fluc[i]);
    mag_force_total += (force_total[i]*force_total[i]);
  }
  mag_force_ev = sqrt(mag_force_ev);
  mag_force_face = sqrt(mag_force_face);
  mag_force_spring = sqrt(mag_force_spring);
  mag_force_bending = sqrt(mag_force_bending);
  mag_force_vol = sqrt(mag_force_vol);
  mag_force_area = sqrt(mag_force_area);
  mag_drag = sqrt(mag_drag);
  mag_force_wall = sqrt(mag_force_wall);
  mag_force_fluc = sqrt(mag_force_fluc);
  mag_force_total = sqrt(mag_force_total);

  sprintf(filename, "%s/data/force_total.dat", work_dir);
  stream = fopen(filename, "a"); 
  fprintf(stream, "%6d  ",step);
  fprintf(stream, "%+.4le    %+.4le    %+.4le    %+.4le   %+.4le    %+.4le    %+.4le    %+.4le    %+.4le    %+.4le\n", 
          mag_force_ev, mag_force_face, mag_force_spring, mag_force_bending, 
          mag_force_vol, mag_force_area, mag_drag, mag_force_wall, mag_force_fluc, 
          mag_force_total);
  fclose(stream);
}
